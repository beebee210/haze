...
def analyze_dark_channel(dark_channel):
    ...
    if mean_val < 80:
        label = "❌ No Haze Detected"
        confidence = 70 + (1 - (mean_val / 80)) * 30  # 70–100%
    elif 80 <= mean_val < 120:
        label = "⚠ Possible Light Haze"
        confidence = 40 + (20 - abs(mean_val - 100)) * (30 / 20)  # 40–70%
    else:
        label = "☁ Heavy Haze Detected"
        confidence = 70 + ((mean_val - 120) / (255 - 120)) * 30  # 70–100%

    return label, mean_val, round(confidence, 2)
...
class VideoTransformer(VideoTransformerBase):
    ...
    def transform(self, frame):
        ...
        _, mean_val, _ = analyze_dark_channel(dark)
        self.haze_values.append(mean_val)

        if len(self.haze_values) >= 5:
            avg_val = np.mean(self.haze_values)
            haze_result, _, confidence = analyze_dark_channel(np.full_like(dark, avg_val))
            self.haze_values.clear()
        else:
            haze_result, confidence = "Calculating...", 0

        cv2.putText(img, f"Haze: {haze_result} ({confidence:.1f}%)", (10, 30),
...
# Optional: TURN server for reliable WebRTC
RTC_CONFIGURATION = RTCConfiguration(
    {"iceServers": [{"urls": ["stun:stun.l.google.com:19302"]}]}  # You can add TURN servers here
)
...
if option == "📁 Upload Image":
    ...
    if uploaded_file is not None:
        ...
        haze_result, mean_val, confidence = analyze_dark_channel(dark)
    ...
    webrtc_streamer(
        ...
        rtc_configuration=RTC_CONFIGURATION,  # ensures reliable connections
